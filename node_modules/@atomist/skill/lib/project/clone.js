...
async function doClone(id, options = {}) {
    log_1.debug(`Cloning repository '${id.owner}/${id.repo}', branch '${id.branch}', sha '${id.sha}' and options '${JSON.stringify(options)}'`);
    const sha = id.sha || "HEAD";
    const repoDir = options.path || path.join(os.tmpdir(), util_1.guid());
    ...
    await pRetry(() => child_process_1.execPromise("git", cloneArgs), retryOptions);
    try {
        await child_process_1.execPromise("git", ["checkout", checkoutRef, "--"], { cwd: repoDir });
    }
    catch (err) {
        // When the head moved on and we only cloned with depth; we might have to do a full clone to get to the commit we want
        log_1.debug(`Ref ${checkoutRef} not in cloned history. Attempting full clone`);
        await child_process_1.execPromise("git", ["fetch", "--unshallow"], { cwd: repoDir });
        await child_process_1.execPromise("git", ["checkout", checkoutRef, "--"], { cwd: repoDir });
    }
    return repoDir;
}
